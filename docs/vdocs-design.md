...
> 16 stages), everything semantic/vector/MCP (§14–§15, `vectors.db`), and all references to
...
DOCUMENT MEDALLION  ── data plane · only the selected subset · turns bytes into corpus + machine views
  🥉 BRONZE   ──fetch──►   raw/<sha256>.docx (write-once)  +  raw/index.json (doc_id → provenance)
...
$LAKE/                                       # DATA_DIR, default ~/data/vdocs, env-overridable
  ...
  inventory/                                 # ── INVENTORY MEDALLION (control plane; metadata only)
    ...
    snapshots/<crawl-date>/                  #   crawl   — VO.2 timeline: one immutable dated copy of
      catalog.raw.json · SNAPSHOT.json       #     bronze per crawl (+ taken_at, canonical_hash, counts)
  ...
  documents/                                 # ── DOCUMENT MEDALLION (data plane; the selected subset)
    ...
    bronze/raw/index.json                    #   fetch   — derived CAS manifest (doc_id → provenance)
  ...
  reports/
    ...
    validation/verification.json             # validate (findings + the cross-run count BASELINE —
                                             #   evidence, so `build --fresh` spares it; R‑14)
    doctor/doctor.json                       # doctor (the GOLD LIBRARY verdict + every check —
                                             #   the gate's record as a computable artifact, P2.1)
```
...
of `acquisitions` ⋈ the gate-admitted targets — **`doc_id` → provenance** (`format: 2`), regenerated
wholesale by `fetch`, not a second source of truth. This is
...
> **P1.1 (2026-08-01): the code now matches this spec.** `fetch` used to *merge* a **sha256**-keyed
> index, which made it a second source of truth that (a) collapsed two logical documents whose DOCX
> bytes were byte-identical — measured live: 1,040 fetched acquisitions → 1,034 entries, six
> documents with no bundle — (b) never dropped a withdrawn document, and (c) could not re-emit an
> entry for a doc already in the CAS (`SKIP_PRESENT` returns before the write). Keying by `doc_id`
> and deriving the whole mapping each run fixes all three: duplicate content yields two entries
> pointing at one CAS blob, membership follows the current admitted set, and a lost entry
> self-heals. A `format: 1` file is refused with a remediation rather than half-read
> (`fetch_pure.parse_raw_index`). Audit: `docs/reference/pipeline-adversarial-audit.md` [S4]/R‑2.
...
|---|---|---|---|---|
| 🥉 INV | **crawl** | `vdl` (external) | `inventory/bronze:catalog.raw` (raw scraped catalog — immutable evidence). **Completeness floor (CI.1, R‑4):** the new yield is compared against the catalog already on disk *before* the write — materially fewer documents (`CRAWL_FLOOR_RATIO`, default 90%) or a previously-populated section going dark fails the stage and leaves the prior catalog in place (fail closed on the artifact); a genuine shrink is accepted loudly with `vdocs crawl --accept-shrink`. **Timeline snapshot (VO.2):** after the write, the crawl is also preserved at `inventory/snapshots/<crawl-date>/` — immutable, never rewritten, and deduplicated against the newest snapshot's **canonical content hash** (sorted rows) so a re-crawl that found the same thing, or the same thing in a different page order, does not fabricate history. Bronze only: gold carries our own classification, so a delta over it would conflate VDL change with a change in how we read it. Compared with `vdocs vdl-delta [before] [after]` (VO.3/VO.4a) | FORCE_ONLY (network) |
...
| 🥇 DOC | **validate** | `text@normalized` (`capture.yaml` + `refs.yaml`) + `consolidated` (each anchor bundle's `bundle.yaml` + parts) + **`gold inventory` + `raw/index.json` + `text@converted` + `registries`** (the acquisition-chain gate's five seams, P1.2) **[sidecar-verification slice — active now]**; `publish`, `index.db`, `vectors.db`, `reports/fidelity` **[deferred, arrive with publish/index/fidelity]**; reads the prior `stage_runs[normalize].counts` (out-of-contract historical baseline for cross-run drop detection, like `fetch`↔`acquisitions`) | `reports/validation` (the verification findings) + (HARD GATE — sets its own `ok`). **Sidecar-verification slice (built first):** (1) **typed-absence gate** — any per-bundle `capture.yaml` outcome of `absent-unexpected` or `failed` blocks; (2) **count reconciliation** — a sidecar class going `absent-expected` corpus-wide, or a count dropping vs. the prior run with no matching source change, is re-classified `absent-unexpected` and blocks; (3) **ref-resolution gate** — every outbound ref in each `refs.yaml` is resolved against that bundle's live anchor set: a **severed** ref (resolved slug matches no live anchor — a true dead anchor) blocks at a hard floor of zero (the C5 TOC round-trip generalised to all cross-refs, §6.7); an **unmapped** ref (`UNRESOLVED` — a Word bookmark `normalize` never mapped) is **reported as a metric, not gated**, split into two classes (triage 2026-06-03): a `_Toc…` bookmark is a TOC field targeting a **heading** → the **C5-bounded, recoverable** resolvability class (mapping lost because Pandoc drops some heading bookmark spans, but reconstructible from the legacy TOC — a tracked `normalize` follow-up — so the C5 ≤0.02 applies to **`_Toc`→heading** resolution, ~0.76 today (534 of 705 on the 1299-doc lake): a known gap, not a regression); a `_Ref…`/other bookmark is a cross-reference to a **non-heading** target (figure/table/numbered item/page span), unmappable to a heading anchor by construction → reported as **expected-unmapped**, **outside** the C5 rate (the `_Ref` class is ~64% of unmapped and resolves 0%, so conflating it miscalibrated the old "~92% expected" framing); (4) **bundle-integrity gate** — for each `consolidated` anchor bundle, recompute every part's `sha256` from disk against its `bundle.yaml`, confirm the on-disk part set matches the manifest exactly (no extra/missing), and recompute `bundle_digest`; any mismatch (tamper / incompleteness) blocks (§6.6 signed manifest). **Lineage extension (P5.2, audit R‑9/[S9]a):** the manifest proves the parts hash to what it *says*, which is precisely why it cannot see a `history.yaml` that **misdescribes** them — so each bundle's lineage is checked independently from the two parts themselves: the member flagged `is_latest` must record `sha256(body.md)`, else a blocking `stale-lineage` finding naming the `doc_id` (a bundle with no lineage, no single `is_latest` member, or no recorded sha is `unverifiable-lineage` — absence is UNKNOWN, never OK); (5) **acquisition-chain gate** (P1.2, audit R-3) — the join no stage performed: `admitted ⊇ fetched == raw_index == converted == normalized`, each seam read from a *different* source (gate policy over the gold inventory · `state.db:acquisitions` · the on-disk index · the two silver trees) so it is independent of any producer's self-report. Every disagreement is a blocking finding naming the **doc_id** (not a count): a document recorded `fetched` that never reached a bundle used to be invisible everywhere — six of them were, measured; (6) **content-retention gate** (P3.3, audit R‑5) — the verdict `normalize` has scored since Phase 3 finally decides what ships. Read off each **gold anchor bundle's** `capture.yaml` (which travels with the anchor under `bundle.yaml`'s signed manifest, so the gate reads a *verified* record): **QUARANTINE always blocks** — a gutted document is not a judgement call, and no registry entry can excuse it; **REVIEW blocks unless** its `doc_id` carries a reason in the curated `registries/retention-signoff.yaml` (the human in the loop is a registry entry, tenet #13, fingerprinted into validate's inputs via `REGISTRIES`); an **unscored** gold bundle blocks (UNKNOWN is never PASS); a sign-off covering a document that no longer needs one is reported as `retention-signoff-stale` but never blocks — registry hygiene, not a corpus defect. **Deferred (full gate):** schema + lineage + ID/vector integrity + the per-document **fidelity verdict** [PASS / REVIEW-with-sign-off only; QUARANTINE blocks], landing with `publish`/`index`/`fidelity`. | ALWAYS_RERUN |
| 🥇 DOC | **doctor** (terminal) | `index.db` (documents + **`relations`**) + **`gold/contract-manifest.json`** (the edge that sorts it after `manifest`, making it the DAG's last node) + `registries` (the doctor policy / gate keep-set / entity-quality declaration the verdict is measured against); `knowledge.db` read **optionally** (absent ⇒ the SKL-projection check is skipped, not failed) | `reports/doctor` (`doctor.json`: `verdict` · `gold_count` · every check's `name`/`health`/`detail` · `generated_at`) + (HARD GATE — RED fails the run, non-zero exit). The 19 soundness checks (coverage floors vs `doctor-policy.yaml`, anchor integrity + coverage, the entity-quarantine cascade, the SKL-projection wipe detector [`index --force` after `merge` silently recreates merge's empty shells], gate fidelity, latest-only FTS, vocab closure, read-contract **verbatim view verification**) — unchanged logic, run through the single `server.doctor.diagnose_lake` path the `vdocs doctor` command also uses. **WARN and BY-DESIGN never fail** (F6: an expected gap is not a defect); they surface as run warnings. P2.1 fixed *placement*, not logic: the gate existed and passed, but ran only under `build` or by hand, so a `vdocs run` could finish green over a RED-able `index.db` (audit R‑6/[S18]). | ALWAYS_RERUN |
...
| `registries/retention-signoff` | documents whose low content-retention verdict a human has judged honest (a title page, a change-pages partial, a pure index) — with a reason and a date | `doc_id` | **EXCUSE** a REVIEW verdict (never QUARANTINE) | human (a reviewed diff — the in-the-loop decision as data, tenet #13) | `validate` (Step 6, P3.3) | — (rules only; a stale entry is reported, never blocking) |
...
registries/        # CURATED, version-controlled pattern catalog (full index: §9.7) — a repo-root sibling of
  ...
  retention-signoff.yaml  # human sign-offs for the content-retention gate → EXCUSE a REVIEW (consumed by validate)
