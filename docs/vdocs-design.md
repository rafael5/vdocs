...
- **Sidecar verification is `validate`'s first slice (built ahead of full `fidelity`).** The typed
  ...
  `stage_runs[counts]` finally consume), and (3) any dead cross-ref anchor in a `refs.yaml`. This is
