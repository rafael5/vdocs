...
- **Sidecar verification is `validate`'s first slice (built ahead of full `fidelity`).** The typed
  ...
  (an `UNRESOLVED` Word bookmark is a reported metric, not gated — ~92% of corpus cross-refs are
  unmapped by design, so a high rate is expected, not a defect). This is
