...
> **REMEDIATED 2026-08-02 (P1–P7).** This is a **point-in-time record of 2026-08-01** and is
> deliberately not rewritten: findings are struck in place with their landing commits, and
> corrections land as dated notes (see the method note at the end). **Read a struck row or a
> `Correction (P7 …)` note before acting on any claim here.** Thirteen of the eighteen register
> rows are now fixed; **R‑18 was added at close-out** for a pattern the audit did not name (a
> stage's output going stale with every input unchanged). What the remediation changed on the live
> lake: chunk-less sections **26.7% → 10.5%**, stale lineage records **615/615 → 0/615**, `doctor`
> advisory → **the DAG's terminal gate (20 checks)**, the acquisition chain gated end-to-end, and
> retrieval quality **+44% mean nDCG@10** (0.3694 → 0.5305 over the 18 answerable golden queries; see R‑19 for the 6 that cannot score). Effort:
> `docs/historical/pipeline-audit-remediation-implementation-plan.md` + its tracker.
>
> ⚠️ **Two corrections to every retrieval figure in this repo.** (1) Anything predating
> 2026-08-02 was measured against the wrong corpus — a stale 451-document `~/data/vdocs-dev`
> lake (this includes `nDCG@10 = 0.469`). (2) Anything quoting **24 labeled queries** is
> depressed by ~13 points: 6 of them cite applications the admission gate excludes and score a
> structural 0.000 (**R‑19**). The figures of record are over the **18 answerable** queries:
> pre-fix **0.3694**, now **0.5305**. See `reports/README.md`.
...
| 9 | `consolidate` | Reconstruct `anchor_key` from FM (same kernel formula as catalog + alias map); group, order (base→patch_num→official_date→slug), collapse to one version-free anchor bundle; retain every body in CAS; append-only `history.yaml`; **signed `bundle.yaml`** (part hashes + digest) written last; stale-part prune | normalized tree + assets | `gold/consolidated/<app>/<stem>/{body.md, history.yaml, bundle.yaml, sidecars, tables/}` + `_shared/history` CAS | `bundle.yaml` is recomputable from disk (validate Step 4 does); deep_gate = error rate | ok: 615 groups from 1,034 docs, 1,034 retained bodies | ~~**High for integrity, Medium for lineage truth** — append-only `merge_history` never refreshes a captured member's facts~~ → **High** **[P7 2026-08-02]**: `75d12e6` supersedes stale facts (measured 615/615 anchors were misdescribing their own body; now 0/615) and `30351e9` gates it in `validate`; the retained-body CAS is checked too (`ae87ef6`) | [S9] |
...
|---|---|---|---|---|
| `vdocs ask` / `server/search.py` | Safe FTS5 MATCH (quoted tokens, OR-joined), field-weighted BM25 (doc_title 2.5 / title 2.0 / path 1.5 / body 1.0), SKL number→name expansion | Golden-set nDCG sweeps recorded in code comments | ~~**Medium** — empty result prints "no matches in the gold corpus." with **none** of the MCP not-indexed warnings~~ → **High** **[P7 2026-08-02]**: `d984f8a` moved the rule + a shared `search_envelope()` into `server/search.py`; the CLI, `--json` and the MCP tool now emit byte-identical text, and `--json` returns the same `{hits, hit_count[, warning]}` envelope | [S16] |
| `vdocs serve-mcp` / `server/mcp.py` | JSON-RPC stdio; tools search/lookup/query/orientation; measured chunk-less rule stated on every surface (26.7% at audit; **10.5%** after `e374d9a` **[P7 2026-08-02]**); `has_indexed_text` answered by probing `chunks`, not trusting `kind` | The NOT_INDEXED_RULE contract; read-only SQL | **High honesty, incomplete capability** — it tells the client to read `body.md` + `tables/*.csv` but cannot serve those bytes itself (see §7) | [S17] |
| `doctor` / `release` / `entity-quality` | **20** soundness checks **[P7 2026-08-02]** (coverage floors, anchor integrity, gate fidelity, latest-only FTS, vocab closure, read-contract verbatim, **published-corpus-card freshness**); release adds quiescence, clean-tree, floors, bundle+sums | GREEN/RED verdict; exit 1 on RED | ~~**High** — …but *advisory* (not in the DAG)~~ → **High and enforced** **[P7 2026-08-02]**: `74b136b` made it the DAG's terminal stage, so `vdocs run` ends on the verdict and RED fails the run | [S18] |
...
[S9]: **Correction (P7, 2026-08-02):** (a) is FIXED — `75d12e6` supersedes a re-processed member's facts instead of discarding them (measured 615/615 anchors stale → 0/615) and `30351e9` gates it; `ae87ef6` extends the gate to the retained-body CAS (2,074 referenced, 0 missing). (b) alphabetical ordering for undated/unpatched groups and (c) no GC of retained bodies (265 orphan blobs measured) are both UNCHANGED and still by design.

**consolidate — gaps.** (a) `merge_history` treats member identity (`doc_id`) as the
...
[S10]: **Correction (P7, 2026-08-02):** FIXED by `e374d9a` — `searchable` is no longer derived from `kind`. The mechanism described below was real, but the framing "container ⇒ no text" was itself the defect: 6,779 of 11,543 containers carried a substantive lead-in, and a second class this footnote missed — 1,896 sections with 1–7 tokens of real prose — was excluded by the 8-token QA floor being reused as a retrieval gate. Chunk-less share 26.7% → **10.5%**.

**index — the 27% problem, mechanism confirmed.** `shred_sections` classifies a
...
[S16]: **Correction (P7, 2026-08-02):** FIXED by `d984f8a` — the rule and a shared `search_envelope()` now live in `server/search.py`, so the CLI, `--json` and the MCP tool emit byte-identical text; `ask --json` returns the same envelope shape.

**ask CLI — gap.** The MCP server states the not-indexed rule on five surfaces; the
...
[S18]: **Correction (P7, 2026-08-02):** FIXED by `74b136b` — `doctor` is the DAG's terminal stage, so `vdocs run` ends on its verdict and RED fails the run. It is now 20 checks (a published-corpus-card freshness check was added in `ae87ef6`).

**doctor/release — placement gap.** 19 checks, all PASS live, including read-contract
...
|----|------|-------|---------------|--------|------------|
| ~~R‑1~~ **FIXED** `1419d1b` (P4.1/P4.2, input half) + `ae87ef6` (R1.1, output half) | Row-count SQLite fingerprints let content-only changes skip consumers (stale index metadata) | §2, fingerprint.py | latent | Wrong published metadata that no gate reds | Content-hash cheap fingerprints (xxhash) or fold a `max(rowid), sum-of-hash` digest; interim: document `--verify` as the post-registry-edit norm |
...
| ~~R‑5~~ **FIXED** `b28fbf5`+`f81dcb2` (P3.1–P3.3) | Retention verdicts don't gate; `blocks_publish` is dead code; QUARANTINE docs ship | normalize, [S8] | 7 low-retention docs in gold | Over-stripped documents presented as gold | Wire QUARANTINE (and un-signed-off REVIEW) into `validate`; credit relocated table words |
| ~~R‑6~~ **FIXED** `74b136b` (P2.1) | Doctor (the only index.db gate) is advisory — `vdocs run` never executes it | [S18] | doctor GREEN, but only because it was run by hand/build | A RED-able corpus can sit under a green pipeline | Make doctor a terminal DAG stage (or auto-append to any run reaching `manifest`) |
| ~~R‑7~~ **FIXED** `e374d9a` (P6.1b) | 27.2% of live sections unsearchable by construction (container lead-ins + hollow) | index, [S10] | 14,153 / 52,048 sections chunk-less | Systematic false negatives; mitigated today only by prompt discipline | Chunk container lead-in text passing the substantive floor (highest-leverage fix in this audit) |
...
| ~~R‑11~~ **FIXED** `d984f8a` (P6.3) | CLI `ask` empty result asserts "no matches in the gold corpus" — the exact false-negative the MCP layer documents against | [S16] | code-confirmed | Agents/scripts on the CLI/JSON path inherit the trap | Emit the shared NOT_INDEXED warning on all three surfaces |
| ~~R‑12~~ **RULED — stop** `SL.2` (2026-08-04) | SKL reach: 6/21 entities reconciled, expansions nearly empty; ~~4,415 unresolved proposals uncurated~~ **the queue is 307 rows — 4,415 was a mention count** | merge/resolve, [S11][S13] | **MEASURED**: 233 files split across a number/name vocabulary; 28 of 80 identifier-shaped questions fail on vocabulary; the layer repairs **3**, ceiling **23**; approving the whole queue adds **1** equivalence (94% of it names entity types the seed cannot hold) | The promised lift is real in the corpus and unreachable through this machinery — and the row's own remedy (curation) was aimed at the wrong half | **The remedy was wrong, not just unbuilt.** The blocker is the **projection** — `skl_expansion_map` drops two-digit keys (`len>=3`) and decimals (`isalnum`, and FTS tokenising `50.7` into `50`/`7`), so no curation loop could have moved it. Claims corrected across the corpus card, user guide, docs index, skl-proposal and skl-implementation-plan; `resolve` now reports `proposal_candidates` + `proposal_mentions` separately so the mention count can never again read as a backlog. Machinery deliberately KEPT: removal is a breaking read-contract change (`v_entity_skl`/`v_entity_synonyms`/`v_chunk_entities`) to recover 33 s of an ~18-min rebuild. See `docs/proposals/vdocs-quality-synonym-layer/sl1-findings.md` |
...
| ~~R‑13~~ **FIXED** `74b136b` (rider with P2.1) | AI-card fingerprint hashes `index.db` main file under WAL — may not reflect reader-visible state | manifest, [S15] | `index.db-wal` present at audit | Staleness detector that can itself be stale | `PRAGMA wal_checkpoint(TRUNCATE)` (or `VACUUM INTO`) before hashing |
| ~~R‑14~~ **FIXED** `74b136b` (rider with P2.1) | `--fresh` wipes `reports/`, deleting the validate drop-detection baseline exactly when everything is rebuilt | validate, [S12] | code-confirmed | The cross-run net is absent on de-novo builds | Exempt `reports/validation/` from the wipe (it is evidence, not derived state — same argument that spared `catalog.raw.json`) |
| R‑18 | **PATTERN (added at P7 close-out, 2026-08-02): a stage's output can go stale with every one of its INPUTS unchanged.** Three distinct mechanisms observed, all now closed: (a) the input fingerprint was too weak to see a content change (`rows:N`); (b) another stage destroyed this stage's output while its inputs sat still (`index` rebuilding `index.db` empties `entity_skl`, which `merge` fills); (c) the output's text lives in **code**, so editing it moves no fingerprint at all (`manifest_pure.USAGE` → `CORPUS.md`) | orchestrator/stage.py, doctor | (a) `1419d1b` (b) `ae87ef6` (c) `ae87ef6` | Each instance shipped a stale artifact under a green pipeline; (b) reached `doctor` RED twice, (c) published a corpus card quoting a number the same commit disproved | Closed by mechanism, not discipline: content hashes · `Stage._clobbered_outputs` in the skip decision · a `doctor` check comparing the published usage rule to the code that renders it. **Left open deliberately:** the *class* is not closed — ask of every new stage, what could make my recorded output untrue while my inputs sit still? |
| R‑19 | **Corpus SCOPE changes are unmeasured, and the golden set rotted against them silently (found 2026-08-02, post-P7).** Three whole applications — XOBW/HWSC (23 inventory docs), KAAJEE (64), LEX (15) — plus `XU/kdc1_0ig` are excluded by the admission gate (`system_type` = *Integration middleware* / *Data patch*, not *VistA*). That is legitimate scope, but **6 of 24 golden queries still cite sections in them**, so they scored a structural 0.000 forever and depressed every published retrieval number by ~13 points (0.3979 reported vs **0.5305** over the 18 answerable queries). ⚠️ **CORRECTED 2026-08-03:** this row first claimed 102 documents had *left* the admitted set unreported. They never entered it — the golden set was curated on the **dev lake**, which admits these applications, and production has `acquisitions = 0` for all three. The residual true finding is narrower but real: nothing tracks the admitted set's composition over time, so a future departure would be equally silent | catalog/fetch gate, golden set | **partially fixed**: `baseline_golden.py` now detects and excludes unscoreable queries and reports the count (`unscoreable_queries`) | A retrieval metric that silently measures label rot instead of engine quality — and, more generally, scope drift that no gate can see | (a) re-label or retire the 6 rotted queries; (b) a cross-run **admitted-set** baseline in `validate` (the drop check it already does for sidecar counts, applied to scope); (c) fold into R‑4's crawl floor — same class, one level up |
