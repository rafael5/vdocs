...
| VO.2 | Immutable dated snapshot on every successful crawl — **bronze only**, deduped by canonical content hash (sorted rows) | ✅ 2026-08-05 | `snapshot_pure.py` (`canonical_hash` over sorted section/app/doc rows, `snapshot_name`, `snapshot_order`) + `_keep_snapshot` in the crawl driver, after the floor check and the bronze write. Each snapshot carries `SNAPSHOT.json` (`taken_at`, `canonical_hash`, counts). **The hash sees an application with no documents** — `Catalog.walk` does not, so it is built from the hierarchy directly. Dedupe is against the **newest** snapshot (a VDL that reverts to an earlier state is still news); `snapshot_order` exists because lexical sort puts `-10` before `-2` and a wrong "newest" would silently drop a real change. 7 unit + 3 integration tests |
| VO.3 | Delta between two snapshots — per-section counts by status, **keyed on `appid`/`secid`**, never parsed names | ✅ 2026-08-05 | `delta_pure.py` + `vdocs vdl-delta [before] [after]` (no args = the two newest). 10 unit + 4 CLI tests. ⚠️ **Keying on ids is not a nicety — it is the only thing that works.** Measured on the real catalog: all 396 `appid`s are distinct, but **350 applications (88%) share their display name with another appid** and **376 (95%) share an `app_code`**. Two different applications are both "Admission Discharge Transfer (ADT)" (appid 55 and 327) |
| VO.4a | Mass-transition tripwire: >5% of apps changing status in one delta → `SUSPECT-PARSER`, transitions suppressed (`app_status` is a display-suffix regex) | ✅ 2026-08-05 | In `vdl_delta`; ratio 5% **and** an absolute floor of 10 (the floor only ever binds on a small crawl — 5% of 396 is ~20). Fires → report leads with `SUSPECT-PARSER`, names the counts and points at `crawl_pure`'s regex, and the transition list is emptied rather than published as history. Verified against the real catalog: a simulated regex break reads as **207 of 395 (52%)** and is suppressed; one genuine transition in the same corpus is reported as one row with no flag |
| VO.1 | *(optional, demoted)* fill-rate report of unused fields — only if VO.3 needs it; CI.0 covered the essentials | ☐ | not needed by VO.3 — not scheduled |
| VO.4b | *(optional)* transition rows in the delta report (status, decommission date, `cots_dependent`) — report rows, **no new channel** | ☐ | status transitions already land as rows (VO.4a needs the count); the open part is the `decommission_date`/`cots_dependent` dimensions, which are **not in bronze** — they arrive at `catalog`/`enrich`. Not scheduled |
| VO.5 | The archived-share question answered with evidence — **closes as composition-answered / intent-unestablished** (proposal Table 1: append the explicit "intent unestablished, and why" paragraph to the findings, then ✅ — no further intent work) | ✅ 2026-08-05 | **Composition answered** (operator hypothesis, measured 2026-08-04): [`vo5-archive-meaning-findings.md`](vo5-archive-meaning-findings.md). **36% of the archive share is VBA benefits forms, not documentation**; of the genuine 2,170, **69.6% are older/duplicate copies** (72.2% older, 25.7% same version, 2.1% newer) and the rest are release-pinned documents. ⚠️ **But archive is NOT redundant in our corpus** — `consolidate` already folds the duplicates, and **0 of the 55 surviving archive documents have an active twin**. **CLOSED 2026-08-05: VA's intent is declared UNESTABLISHED, and the old "needs VO.2–VO.4" deferral was wrong** — the timeline starts 2026-08-05 while the labels in question were applied **2005–2022** (all 115 `decommission_date` values predate the first snapshot), so that evidence cannot arrive by construction. Only a statement from VA would settle it. No further intent work is scheduled |
| VO ✓ | VO.0, VO.2, VO.3, VO.4a and VO.5 all ✅ (proposal Table 1 DoDs) — optional rows not required | ✅ 2026-08-05 | **Table 1 complete.** 1,488 tests (+25), coverage 96.39%, `make check` exit 0. ⚠️ **One defect caught only by running the finished command against the real lake:** VO.0 was banked by hand into `bronze/`+`gold/` subdirectories, a shape VO.2's reader does not write — so the first snapshot was invisible to `vdl-delta`, and a stray directory sorting last would be read as the newest, record no hash, and **silently defeat deduplication on every future crawl**. Fixed by one rule in both places: *a snapshot is a directory holding a catalog, not merely a directory*; the banked snapshot now also carries the canonical `catalog.raw.json` + `SNAPSHOT.json` (bronze bytes verbatim, `SHA256SUMS` still verifies). Timeline is live: one banked snapshot, automatic capture on every future crawl, `vdocs vdl-delta` compares any two without re-crawling. ⚠️ **A second snapshot only exists after the next crawl** — until then `vdl-delta` correctly reports it has one and exits 1 |
