...
| VO.8b | ⛔ **BLOCKER before admitting** — table-shaped TOCs survive `strip_legacy_toc` | ☐ | Docling renders each legacy TOC as a markdown table (rows often 4×-duplicated); `_is_toc_nav_line` returns False for a table row, so the strip bounds immediately. Kernel DG: dot-leaders **3,417 → 3,416**, **1** entry captured to `toc.yaml`. Across the 19: **49,105 words (5.5%)** of dot-leader TOC text, 11.9% of the Kernel DG body. **Safe failure — content retained, not deleted; capture-before-strip holds.** Fix = widen the detector, not a new mechanism |
| VO.8c | Heading depth from section numbering | ☐ | **15/19 documents are single-level.** Not an export artifact — Docling's JSON assigns every `section_header` `level: 1`, and `infer_heading_levels` removes gaps rather than creating hierarchy. Recoverable where headings are numbered (48 of ~58 in the DIBR guides; 0 in the IGs). Degrades `section_path`, which RR.2 showed is a live ranking signal |
