...
| VO ✓ | VO.0, VO.2, VO.3, VO.4a and VO.5 all ✅ (proposal Table 1 DoDs) — optional rows not required | ✅ 2026-08-05 | **Table 1 complete.** 1,487 tests (+24), coverage 96.40%, `make check` exit 0. Timeline is live: one banked snapshot, automatic capture on every future crawl, `vdocs vdl-delta` compares any two without re-crawling. ⚠️ **A second snapshot only exists after the next crawl** — until then `vdl-delta` correctly reports it has one and exits 1 |
