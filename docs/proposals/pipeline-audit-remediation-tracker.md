...
| P4.1 | `sqlite_fingerprint` always content-hash (canonical row encoding) | ☐ | **Cost measured 2026-08-01, decisively bounded:** largest contracted table `index.db:relations` **203,272 rows → 0.33 s**; all 13 contracted tables ≈ **1.1 s** total. No memoization needed. |
| P4.2 | `rows:N` legacy-format migration (accept-once, re-record) | ☐ | **13 of 31** recorded `outputs_fp` are still `rows:N` — every SQLite producer (serve-inventory, enrich, index, resolve, merge, relate). |
| P4 ✓ | Same-rowcount cell mutation re-runs `index`; full live run, no spurious drift FAILs | ☐ | **The defect fired live during P3's acceptance run** — see the note below. |
