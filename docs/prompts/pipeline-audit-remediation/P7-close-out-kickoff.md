...
`corpus_content_hash 6dbec1f5…` · `vdocs run` **GREEN 16/16** at doctor (19 pass / 0 fail) ·
`validate` GREEN (chain 0, reconcile 0, retention 0, severed 0, bundle 0 incl. lineage) ·
`make check` green at **1,205 tests / 96.31%**.
...
**Expect the `merge`/`entity_skl` trap** (below) and budget for it.
...
Status/Credibility cells in the master table that these fixes change. **Do not strike R‑1 outright**
— see below.
...
## Two things that are NOT closed — decide them explicitly

1. **R‑1 is only half closed.** P4 fixed the *input* half (a content change now reaches a consumer).
   The *output* half is live: when `index` rebuilds content-identically, `merge` correctly sees
   unchanged inputs and skips — while `index` has already recreated `entity_skl` as an empty shell,
   so `doctor` goes RED. It recurred during P5's acceptance run, *after* P4. Repair is
   `vdocs run --from merge --force`. **Either fix it in P7** (a stage verifying its own outputs
   exist, or `index` not owning a table `merge` fills) **or leave R‑1 open with this note** — but do
   not strike it as done.
2. **A stage's output can go stale with every input unchanged — three distinct ways so far**
   (weak fingerprint · another stage wiped it · the text lives in code, so `manifest_pure.USAGE`
   needed a `contract_ver` bump to republish `CORPUS.md`). That is a *pattern*, not three
   coincidences. Worth one register row of its own rather than three footnotes.
...
- R‑1's residual is either fixed or explicitly left open with the mechanism written down.
